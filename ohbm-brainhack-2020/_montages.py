...
from mne.transforms import (apply_trans, get_ras_to_neuromag_trans, _sph_to_cart,
                          _topo_to_sph, _frame_to_str, Transform)#,
                          #_verbose_frames, _fit_matched_points,
                          #_quat_to_affine)
from mne.io._digitization import (_count_points_by_type,
...
from mne.utils import (warn, copy_function_doc_to_method_doc, _pl,
                     _check_option, _validate_type, _check_fname,
                     fill_doc, logger, deprecated)

from mne.channels._dig_montage_utils import _read_dig_montage_egi
from mne.channels._dig_montage_utils import _parse_brainvision_dig_montage
...
def read_dig_hpts(fname, unit='mm'):
    ...
    return make_dig_montage(ch_pos=ch_pos, nasion=fid['nasion'],lpa=fid['lpa'],rpa=fid['rpa'], hpi=hpi, hsp=hsp)
