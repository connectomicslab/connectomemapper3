...
def _infer_kind(bids_basename, bids_root, sub, ses):
    ...
    if not kinds:
        ...
        msg = ('Found data of more than one recording modality. Please pass the `kind` parameter to specify which data to load. Found the following kinds:',kinds)
        raise RuntimeError(msg)
...
def _get_bids_fname_from_filesystem(bids_basename, bids_root, sub, ses,
    ...
    bti_dir = op.join(bids_root, data_dir, bids_basename,'_',kind)
    if op.isdir(bti_dir):
        logger.info('Assuming BTi data in' + bti_dir)
        bids_fname = bti_dir+'.pdf'
    else:
...
def _make_bids_fname(bids_basename, bids_root=None, kind=None, extension=None,
    ...
    if extension is None:
        ...
        bids_fname = bids_basename+'_'+kind+'.'+extension
...
def read_raw_bids(bids_basename, bids_root, kind=None, extra_params=None,
    ...
    acq = params['acq']
    search_modifier = 'acq-'+acq if acq else ''
    elec_suffix = search_modifier+'*_electrodes.tsv'
    coord_suffix = search_modifier+'_coordsystem.json'
...
def get_matched_empty_room(bids_basename, bids_root):
    ...
    if failed_to_get_er_date_count > 0:
        msg = ('Could not retrieve the empty-room measurement date from '
               'a total of' +failed_to_get_er_date_count+ ' recording(s).')
        warn(msg)
