...
def _get_kinds_for_sub( bids_basename, bids_root, sub, ses=None):
    ...
    subject_dir = op.join(bids_root, 'sub-'+sub)
    if ses is not None:
        subject_dir = op.join(subject_dir, 'ses-'+ses)
...
def get_entity_vals(bids_root, entity_key,arg, ignore_sub='emptyroom',
    ...
    for filename in Path(bids_root).rglob('*{}-*_*'.format(entity_key)):
        if ignore_sub and any([filename.stem.startswith('sub-'+s+'_')
                               for s in ignore_sub]):
            continue
        if ignore_task and any(['_task-'+t+'_' in filename.stem
                                for t in ignore_task]):
            continue
        if ignore_ses and any(['_ses-'+s+'_' in filename.stem
                               for s in ignore_ses]):
            continue
        if ignore_run and any(['_run-'+r+'_' in filename.stem
                               for r in ignore_run]):
            continue
        if ignore_acq and any(['_acq-'+a+'_' in filename.stem
                               for a in ignore_acq]):
            continue
...
def _gen_bids_basename( subject=None, session=None, task=None,
    ...
    if on_invalid_er_session not in ['raise', 'warn', 'continue']:
        msg = ('on_invalid_er_session must be raise, warn, or continue, '
               'but received: '+ on_invalid_er_session)
        raise ValueError(msg)

    if on_invalid_er_task not in ['raise', 'warn', 'continue']:
        msg = ('on_invalid_er_task must be raise, warn, or ignore, '
               'but received: '+ on_invalid_er_task)
        raise ValueError(msg)
    ...
    if subject == 'emptyroom':
        if task != 'noise':
            msg = ('task must be "noise" if subject is "emptyroom", but '
                   'received: ' +task)
        ...
        try:
            ...
            msg = ('empty-room session should be a string of format '
                   'YYYYMMDD, but received: '+session)
            if on_invalid_er_session == 'raise':
                raise ValueError(msg)
            elif on_invalid_er_session == 'warn':
                msg = msg+'. Will proceed anyway, but you should consider fixing your dataset.'
                logger.critical(msg)
            else:
                pass
