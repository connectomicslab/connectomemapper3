    ...
    def define_inspect_outputs(self):
        ...
        if self.config.pipeline == "Diffusion":
                if(os.path.exists(target_path) and os.path.exists(reg_results_path) and os.path.exists(warpedROIVs_results_path) and os.path.exists(warped5TT_results_path) and os.path.exists(warpedPVEs_results_path) and os.path.exists(warpedWM_results_path) and os.path.exists(warpedT1_results_path)):
                    ...
                    if self.config.registration_mode == 'FSL':
                        if(os.path.exists(fnirt_results_path)):
                                ...
                                self.inspect_outputs_dict['Linear T1-to-b0'] = ['fsleyes','-sdefault',reg_results.inputs['reference'],reg_results.outputs.out_file,'-cm',"copper",'-a','50']
                                self.inspect_outputs_dict['Wrapped T1-to-b0'] = ['fsleyes','-sdefault',fnirt_results.inputs['ref_file'],T1_results.outputs.out_file,'-cm',"copper",'-a','50']
                                self.inspect_outputs_dict['Wrapped 5TT-to-b0'] = ['fsleyes','-sdefault',fnirt_results.inputs['ref_file'],mrtrix_5tt_results.outputs.output_image,'-cm',"hot",'-a','50']
                                ...
                                if type(rois_results.outputs.out_files) == str:
                                        self.inspect_outputs_dict['%s-to-b0' % os.path.basename(rois_results.outputs.out_files)] = ['fsleyes','-sdefault',fnirt_results.inputs['ref_file'],rois_results.outputs.out_files,'-cm','random','-a','50']
                                else:
                                    for roi_output in rois_results.outputs.out_files:
                                            self.inspect_outputs_dict['%s-to-b0' % os.path.basename(roi_output)] = ['fsleyes','-sdefault',fnirt_results.inputs['ref_file'],roi_output,'-cm','random','-a','50']

                                if type(pves_results.outputs.out_files) == str:
                                        self.inspect_outputs_dict['%s-to-b0' % os.path.basename(pves_results.outputs.out_files)] = ['fsleyes','-sdefault',fnirt_results.inputs['ref_file'],pves_results.outputs.out_files,'-cm','hot','-a','50']
                                else:
                                    for pve_output in pves_results.outputs.out_files:
                                            self.inspect_outputs_dict['%s-to-b0' % os.path.basename(pve_output)] = ['fsleyes','-sdefault',fnirt_results.inputs['ref_file'],pve_output,'-cm','hot','-a','50']
                            ...
                            self.inspect_outputs_dict['Linear T1-to-b0'] = ['fsleyes','-sdefault',reg_results.inputs['fixed_image'][0],reg_results.outputs.warped_image,'-cm','copper','-a','50']
                            ...
                            if self.config.ants_perform_syn:
                                if(os.path.exists(syn_results_path)):
                                    ...
                                    self.inspect_outputs_dict['Wrapped T1-to-b0'] = ['fsleyes','-sdefault',syn_results.inputs['fixed_image'][0],T1_results.outputs.output_image,'-cm','copper','-a','50']
                                    #self.inspect_outputs_dict['Deformation field'] = ['fsleyes','-sdefault',fnirt_results.outputs.fieldcoeff_file]#['mrview',fa_results.inputs['ref_file'],'-vector.load',fnirt_results.outputs.fieldcoeff_file]#
                            ...
                            self.inspect_outputs_dict['Wrapped 5TT-to-b0'] = ['fsleyes','-sdefault',reg_results.inputs['fixed_image'][0],mrtrix_5tt_results.outputs.output_image,'-cm','copper','-a','50']

                            if type(rois_results.outputs.output_images) == str:
                                if self.config.ants_perform_syn:
                                    if(os.path.exists(syn_results_path)):
                                        self.inspect_outputs_dict['%s-to-b0' % os.path.basename(rois_results.outputs.output_images)] = ['fsleyes','-sdefault',syn_results.inputs['fixed_image'][0],rois_results.outputs.output_images,'-cm','random','-a','50']
                                else:
                                    self.inspect_outputs_dict['%s-to-b0' % os.path.basename(rois_results.outputs.output_images)] = ['fsleyes','-sdefault',reg_results.inputs['fixed_image'][0],rois_results.outputs.output_images,'-cm','random','-a','50']
                            else:
                                for roi_output in rois_results.outputs.output_images:
                                    if self.config.ants_perform_syn:
                                        if(os.path.exists(syn_results_path)):
                                            self.inspect_outputs_dict['%s-to-b0' % os.path.basename(roi_output)] = ['fsleyes','-sdefault',syn_results.inputs['fixed_image'][0],roi_output,'-cm','random','-a','50']
                                    else:
                                        self.inspect_outputs_dict['%s-to-b0' % os.path.basename(roi_output)] = ['fsleyes','-sdefault',reg_results.inputs['fixed_image'][0],roi_output,'-cm','random','-a','50']

                            if type(pves_results.outputs.output_images) == str:
                                if self.config.ants_perform_syn:
                                    if(os.path.exists(syn_results_path)):
                                        self.inspect_outputs_dict['%s-to-b0' % os.path.basename(pves_results.outputs.output_images)] = ['fsleyes','-sdefault',syn_results.inputs['fixed_image'][0],pves_results.outputs.output_images,'-cm','hot','-a','50']
                                else:
                                    self.inspect_outputs_dict['%s-to-b0' % os.path.basename(pves_results.outputs.output_images)] = ['fsleyes','-sdefault',reg_results.inputs['fixed_image'][0],pves_results.outputs.output_images,'-cm','hot','-a','50']
                            else:
                                for pve_output in pves_results.outputs.output_images:
                                    if self.config.ants_perform_syn:
                                        if(os.path.exists(syn_results_path)):
                                            self.inspect_outputs_dict['%s-to-b0' % os.path.basename(pve_output)] = ['fsleyes','-sdefault',syn_results.inputs['fixed_image'][0],pve_output,'-cm','hot','-a','50']
                                    else:
                                        self.inspect_outputs_dict['%s-to-b0' % os.path.basename(pve_output)] = ['fsleyes','-sdefault',reg_results.inputs['fixed_image'][0],pve_output,'-cm','hot','-a','50']
        else:
            if (os.path.exists(target_path)) and (os.path.exists(reg_results_path)) and (os.path.exists(warpedROIVs_results_path)):
                ...
                self.inspect_outputs_dict['Mean-fMRI/T1-to-fMRI'] = ['fsleyes','-sdefault',target.inputs['in_file'],reg_results.outputs.out_file,'-cm','copper','-a','50']

                if type(rois_results.outputs.out_files) == str:
                        self.inspect_outputs_dict['Mean-fMRI/%s' % os.path.basename(rois_results.outputs.out_files)] = ['fsleyes','-sdefault',target.outputs.out_file,rois_results.outputs.out_files,'-cm','random','-a','50']
                else:
                    for roi_output in rois_results.outputs.out_files:
