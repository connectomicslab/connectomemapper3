    ...
    def create_workflow(self, flow, inputnode, outputnode):
        ...
        if self.config.registration_mode == 'FSL':
            ...
            mr_convert_FA_unmasked = pe.Node(
                interface=MRConvert(out_filename='fa_corrected_unmasked.nii.gz', stride=[+1, +2, +3]),
            ...
            FA_noNaN_unmasked = pe.Node(
                interface=cmp_fsl.MathsCommand(out_file='fa_corrected_unmasked_nonan.nii.gz', nan2zeros=True),
            ...
            fsl_applyxfm_wm = pe.Node(
                interface=fsl.ApplyXFM(apply_xfm=True, interp="nearestneighbour", out_file="wm_mask_registered.nii.gz"),
            ...
            fsl_applyxfm_brain_mask = pe.Node(
                interface=fsl.ApplyXFM(apply_xfm=True, interp="spline", out_file="brain_mask_registered_temp.nii.gz"),
            ...
            fsl_applyxfm_brain = pe.Node(
                interface=fsl.ApplyXFM(apply_xfm=True, interp="spline", out_file="brain_registered.nii.gz"),
                name="apply_registration_brain")
            fsl_applyxfm_T1 = pe.Node(
                interface=fsl.ApplyXFM(apply_xfm=True, interp="spline", out_file="T1_registered.nii.gz"),
            ...
            mr_threshold_T1 = pe.Node(
                interface=MRThreshold(abs_value=10, out_file='T1_registered_th.nii.gz', quiet=True, force_writing=True),
                name="mr_threshold_T1")
            
            fsl_applyxfm_5tt = pe.Node(
                interface=fsl.ApplyXFM(apply_xfm=True, interp="spline", out_file="5tt_registered.nii.gz"),
                name="apply_registration_5tt")
            fsl_applyxfm_gmwmi = pe.Node(
                interface=fsl.ApplyXFM(apply_xfm=True, interp="spline", out_file="gmwmi_registered.nii.gz"),
                name="apply_registration_5tt")
            ...
            ants_applywarp_T1 = pe.Node(
                interface=ants.ApplyTransforms(default_value=0, interpolation="Gaussian", out_postfix="_warped"),
                name="apply_warp_T1")
            ants_applywarp_brain = pe.Node(
                interface=ants.ApplyTransforms(default_value=0, interpolation="Gaussian", out_postfix="_warped"),
                name="apply_warp_brain")
            ants_applywarp_brainmask = pe.Node(
                interface=ants.ApplyTransforms(default_value=0, interpolation="NearestNeighbor", out_postfix="_warped"),
            ...
            ants_applywarp_pves = pe.Node(
                interface=MultipleANTsApplyTransforms(interpolation="Gaussian", default_value=0, out_postfix="_warped"),
        ...
        if self.config.registration_mode == 'BBregister (FS)':
            ...
            flow.connect([
                (inputnode, fs_bbregister,
                 [(('subjects_dir', unicode2str), 'subjects_dir'), (('subject_id', os.path.basename), 'subject_id')]),
