            ...
            if(os.path.exists(parc_results_path)):
                ...
                if isinstance(parc_results.outputs.roi_files_in_structural_space, (str,unicode)):
                    ...
                    if self.config.parcellation_scheme == 'Lausanne2008':
                        ...
                        finalparc_results_path = os.path.join(self.stage_dir,"parcCombiner","result_parcCombiner.pklz")

                        if(os.path.exists(finalparc_results_path)):
                            finalparc_results = pickle.load(gzip.open(finalparc_results_path))

                            for roi_v, lut_file in zip(finalparc_results.outputs.output_rois,finalparc_results.outputs.colorLUT_files):
                                roi_basename = os.path.basename(roi_v)
                                self.inspect_outputs_dict[roi_basename] = ['freeview','-v',
                                                                                   white_matter_file+':colormap=GEColor',
                                                                                   roi_v+":colormap=lut:lut="+lut_file]
                        ...
                        parc_results_path = os.path.join(self.stage_dir,"Lausanne2018_parcellation","result_Lausanne2018_parcellation.pklz")
                        brain = None
                        if(os.path.exists(parc_results_path)):
                            parc_results = pickle.load(gzip.open(parc_results_path))
                        ...
                        if(os.path.exists(results_path)):
                            ...
                            self.inspect_outputs_dict['Thalamic nuclei - Probability maps'] = ['fsleyes','-sdefault',results.inputs['T1w_image'],results.outputs.prob_maps_registered,'-cm',"copper",'-a','50']
                            self.inspect_outputs_dict['Thalamic nuclei - MaxProb labels'] = ['fsleyes','-sdefault',results.inputs['T1w_image'],results.outputs.max_prob_registered,"-cm","render3",'-a','50']
                        ...
                        if(os.path.exists(results_path)):
                            ...
                            if(os.path.exists(brain)):
                                self.inspect_outputs_dict['Brainstem structures'] = ['fsleyes','-sdefault',brain,results.outputs.brainstem_structures,"-cm","random", '-a','50']
                            else:
                                self.inspect_outputs_dict['Brainstem structures'] = ['fsleyes','-sdefault',results.outputs.brainstem_structures,"-cm","random"]
                        ...
                        if(os.path.exists(results_path)):
                            ...
                            if(os.path.exists(brain)):
                                self.inspect_outputs_dict['Hippocampal subfields'] = ['fsleyes','-sdefault',
                                                                                        brain,
                                                                                        results.outputs.lh_hipposubfields,"-cm","random",'-a','50',
                                                                                        results.outputs.rh_hipposubfields,"-cm","random",'-a','50']
                            else:
                                self.inspect_outputs_dict['Hippocampal subfields'] = ['fsleyes','-sdefault',
                                                                                        results.outputs.lh_hipposubfields,"-cm","random",
                                                                                        results.outputs.rh_hipposubfields,"-cm","random"]
