    ...
    def define_inspect_outputs(self):
        ...
        if self.config.slice_timing:
            ...
            if(os.path.exists(slc_timing_path)):
                ...
                self.inspect_outputs_dict['Slice time corrected image'] = ['fsleyes','-ad',slice_results.outputs.slice_time_corrected_file,'-cm','brain_colours_blackbdy_iso']
            if self.config.motion_correction:
                ...
                if(os.path.exists(motion_results_path)):
                    ...
                    self.inspect_outputs_dict['Slice time and motion corrected image'] = ['fsleyes','-ad',motion_results.outputs.out_file,'-cm','brain_colours_blackbdy_iso']

        elif self.config.motion_correction:
            motion_results_path = os.path.join(self.stage_dir,"motion_correction","result_motion_correction.pklz")
            if(os.path.exists(motion_results_path)):
                motion_results = pickle.load(gzip.open(motion_results_path))
                self.inspect_outputs_dict['Motion corrected image'] = ['fsleyes','-ad',motion_results.outputs.out_file,'-cm','brain_colours_blackbdy_iso']
