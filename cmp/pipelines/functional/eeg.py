...
from cmp.stages.preprocessing.eeg_preprocessing import PreprocessingStage

# from cmp.stages.preprocessing.eeg_roidipole import EEGROIDipoleStage
# from cmp.stages.preprocessing.eeg_inverse import EEGInverseSolutionsStage
# from cmp.stages.connectome.eeg_connectome import ConnectomeStage


class Global_Configuration(HasTraits):
    process_type = Str('SEEG')
    imaging_model = Str


class Check_Input_Notification(HasTraits):
    message = Str
    imaging_model_options = List(['SEEG'])
    imaging_model = Str

class seegPipeline(Pipeline):
    now = datetime.datetime.now().strftime("%Y%m%d_%H%M")
    pipeline_name = Str("seeg_pipeline")
    ...
    ordered_stage_list = ['EEGROIDipoleStage', 'Connectome']
