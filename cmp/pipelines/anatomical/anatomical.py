    ...
    def create_pipeline_flow(
        ...
        if self.parcellation_scheme == "Lausanne2008":
            sinker.inputs.substitutions = [
                ...
                (
                    "wm_eroded.nii.gz",
                    self.subject + "_label-WM_desc-eroded_dseg.nii.gz",
                ),
                (
                    "csf_eroded.nii.gz",
                    self.subject + "_label-CSF_desc-eroded_dseg.nii.gz",
                ),
                (
                    "brain_eroded.nii.gz",
            ...
            sinker.inputs.substitutions = [
                ...
                (
                    ...
                    self.subject + "_atlas-Desikan_dseg.graphml",
                ),
                (
        ...
        if self.stages["Segmentation"].enabled:
            if self.stages["Segmentation"].config.seg_tool == "Freesurfer":
                ...
                if (
                    self.stages["Segmentation"].config.use_existing_freesurfer_data
                    is False
                ):
                    self.stages[
                        "Segmentation"
