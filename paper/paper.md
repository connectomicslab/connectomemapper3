...
\textbf{A Graphical User Interface that reflects the worklow structure} CMP3 takes advantage of Traits/TraitsUI
...
\textbf{Highly reusable connectome mapping outputs}
...
The data derived from the processing of one subject are placed in a `sub-<participant_id>/`
  folder.
Derivatives are also kept in separate folders to distinguish main CMP3 outputs (`cmp-<version>/`) from
  outputs produced by FreeSurfer (`freesurfer-<version>/`) and from all intermediate workflow outputs
...
Main sMRI-, dMRI-, and rfMRI-derived data, which includes preprocessed data, brain parcellations,
  ...
  folder, and which contains `sub-<subject_label>_log.txt`, a log file that
  summarizes output execution.
All intermediate outputs generated by nipype interfaces can be found in their respective
  `<pipeline_name>/` `<stage_name>/` `<interface_name>/` folders, which contains among others execution
  report in the `<interface_name>/` folder, and the pipeline execution graph in the `<pipeline_name>/`
  folder.
