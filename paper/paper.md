...
All intermediate outputs generated by nipype interfaces can be found in their respective
  `<pipeline_name>/`, `<stage_name>/`, and `<interface_name>/` folders, which contains among others
  an image of the pipeline execution graph in the `<pipeline_name>/` folder, and an interface execution
  report in each `<interface_name>/` folder.
