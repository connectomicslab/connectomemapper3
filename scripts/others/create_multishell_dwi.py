...
def main(dwi_files, json_files, bval_files, bvec_files, dwi_out_file, json_out_file, bval_out_file, bvec_out_file):
    ...
    for i, (dwi_file, json_file, bval_file, bvec_file) in enumerate(
        ...
        if i == 0:
            ...
            with open(json_file) as f:
                ...
                json_out_data['global']['const']['SAR'].append(data['global']['const']['SAR'])
    ...
    out_img = nib.Nifti1Image(dwi_out_data, header=out_header, affine=img.affine)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Create multi-shell DWI by combining single shell DWIs')
    parser.add_argument('--dwi', type=str, nargs='+', help='Path to each single shell DWI nifti to be combined')
    parser.add_argument('--json', type=str, nargs='+', help='Path to each single shell DWI json to be combined')
    ...
    parser.add_argument('--dwi_output', help='Output filename of the combined multi-shell DWI nifti')
    parser.add_argument('--json_output', help='Output filename of the combined multi-shell DWI json')
    parser.add_argument('--bval_output', help='Output filename of the combined multi-shell DWI bval')
    parser.add_argument('--bvec_output', help='Output filename of the combined multi-shell DWI bvec')
