...
def refresh_folder(derivatives_directory, subject, input_folders):
    ...
    paths.append(os.path.join(derivatives_directory,'cmp3',subject))

    for in_f in input_folders:
        paths.append(os.path.join(derivatives_directory,'cmp3',subject,in_f))

    paths.append(os.path.join(derivatives_directory,'cmp3',subject,'tmp','nipype'))
...
def update_last_processed(project_info, pipeline):
    # last date
    if os.path.exists(os.path.join(project_info.base_directory,'derivatives','cmp3',project_info.subject)):
        out_dirs = os.listdir(os.path.join(project_info.base_directory,'derivatives','cmp3',project_info.subject))
        # for out in out_dirs:
    ...
    # last stage
    if os.path.exists(os.path.join(project_info.base_directory,'derivatives','cmp3',project_info.subject,'tmp','nipype',project_info.process_type+'_pipeline')):
        stage_dirs = []
        for root, dirnames, _ in os.walk(os.path.join(project_info.base_directory,'derivatives','cmp3',project_info.subject,'tmp','nipype')):
