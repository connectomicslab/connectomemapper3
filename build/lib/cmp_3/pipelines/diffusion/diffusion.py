...
class DiffusionPipeline(Pipeline):
    ...
    def check_input(self, gui=True):
        ...
        mem = Memory(base_dir=os.path.join(self.derivatives_directory,'cmp3',self.subject,'tmp','nipype'))
        swap_and_reorient = mem.cache(SwapAndReorient)            
    ...
    def process(self):
        ...
        deriv_subject_directory = os.path.join(self.base_directory,"derivatives","cmp3",self.subject)
        ...
        #templates =    {"T1": "derivatives/cmp3/{subject}/anat/{subject}_T1w_reo.nii.gz",
        #                "T2": "derivatives/cmp3/{subject}/anat/{subject}_T2w_reo.nii.gz",
        ...
        outdir = os.path.join(self.base_directory,"derivatives","cmp3",self.subject)
        if not os.path.exists(outdir):
            os.makedirs(outdir)
        shutil.copy(self.config_file,outdir)
        #shutil.copy(os.path.join(self.base_directory,"derivatives","cmp3",self.subject,'pypeline.log'),outdir)
    ...
    def old_process(self):
        ...
        # copy .ini and log file
        outdir = os.path.join(self.base_directory,"derivatives","cmp3",self.subject,"connectome",now)
        if not os.path.exists(outdir):
            os.makedirs(outdir)
        shutil.copy(self.config_file,outdir)
        shutil.copy(os.path.join(self.base_directory,"derivatives","cmp3",self.subject,'pypeline.log'),outdir)
