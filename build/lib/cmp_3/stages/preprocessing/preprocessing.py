...
class PreprocessingStage(Stage):
    ...
    def create_workflow(self, flow, inputnode, outputnode):
        ...
        mr_convert = pe.Node(interface=mrt.MRConvert(out_filename='diffusion.mif',stride=[-1,-2,+3,+4]), name='mr_convert')
        ...
        mr_convert_brainmask = pe.Node(interface=mrt.MRConvert(out_filename='brainmaskfull.nii.gz',stride=[-1,2,3],output_datatype='float32'),name='mr_convert_brain_mask')
        mr_convert_brain = pe.Node(interface=mrt.MRConvert(out_filename='anat_masked.nii.gz',stride=[-1,2,3],output_datatype='float32'),name='mr_convert_brain')
        mr_convert_T1 = pe.Node(interface=mrt.MRConvert(out_filename='anat.nii.gz',stride=[-1,2,3],output_datatype='float32'),name='mr_convert_T1')
        mr_convert_roi_volumes = pe.Node(interface=mrt.MRConvert(out_filename='roi_volumes.nii.gz',stride=[-1,2,3],output_datatype='float32'),name='mr_convert_roi_volumes')
        mr_convert_wm_mask_file = pe.Node(interface=mrt.MRConvert(out_filename='wm_mask_file.nii.gz',stride=[-1,2,3],output_datatype='float32'),name='mr_convert_wm_mask_file')
        ...
        if self.config.denoising:
            dwi_denoise = pe.Node(interface=mrt.DWIDenoise(out_file='diffusion_denoised.mif',out_noisemap='diffusion_noisemap.mif') , name='dwi_denoise')
        ...
        mr_convert_b0 = pe.Node(interface=mrt.MRConvert(out_filename='b0.nii.gz',stride=[-1,-2,+3]), name='mr_convert_b0')
        ...
        mr_convert_b = pe.Node(interface=mrt.MRConvert(out_filename='diffusion_corrected.nii.gz',stride=[-1,-2,+3,+4]),name='mr_convert_b')
       
        if self.config.bias_field_correction:
            if self.config.bias_field_algo == "ANTS N4":
                dwi_biascorrect = pe.Node(interface=mrt.DWIBiasCorrect(use_ants=True,out_bias='diffusion_denoised_biasfield.mif'), name='dwi_biascorrect')
            elif self.config.bias_field_algo == "FSL FAST":
                dwi_biascorrect = pe.Node(interface=mrt.DWIBiasCorrect(use_fsl=True,out_bias='diffusion_denoised_biasfield.mif'), name='dwi_biascorrect')
        ...
        if self.config.eddy_current_and_motion_correction:
            eddy_correct = pe.Node(interface=fsl.EddyOpenMP(out_file="eddy_corrected.nii.gz",verbose=True),name='eddy')
            flow.connect([
