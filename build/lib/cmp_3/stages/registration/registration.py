...
class RegistrationStage(Stage):
    ...
    def create_workflow(self, flow, inputnode, outputnode):        
        ...
        if self.config.registration_mode == 'Linear + Non-linear (FSL)':
            ...
            mr_convert_FA = pe.Node(interface=mrt.MRConvert(out_filename='fa_corrected.nii.gz',stride=[-1,-2,+3]), name='mr_convert_FA')
            mr_convert_FA_unmasked = pe.Node(interface=mrt.MRConvert(out_filename='fa_corrected_unmasked.nii.gz',stride=[-1,-2,+3]), name='mr_convert_FA_unmasked')

            FA_noNaN = pe.Node(interface=fsl.MathsCommand(out_file='fa_corrected_nonan.nii.gz',nan2zeros=True),name='FA_noNaN')
            FA_noNaN_unmasked = pe.Node(interface=fsl.MathsCommand(out_file='fa_corrected_unmasked_nonan.nii.gz',nan2zeros=True),name='FA_noNaN_unmasked')

            flow.connect([
                (mr_convert,dwi2tensor,[('converted','in_file')]),
                (inputnode,dwi2tensor,[('target_mask','in_mask_file')]),
                (dwi2tensor,tensor2FA,[('tensor','in_file')]),
                (inputnode,tensor2FA,[('target_mask','in_mask_file')]),
                (tensor2FA,mr_convert_FA,[('FA','in_file')]),
            ...
            flow.connect([
                ...
                (tensor2FA_unmasked,mr_convert_FA_unmasked,[('FA','in_file')]),
