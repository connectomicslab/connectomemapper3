#!/bin/bash
#
# Collects the pull-requests since the latest release and
# aranges them in the CHANGES.rst txt file.
#
# This is a script to be run before releasing a new version.
#
# Usage /bin/bash update_changes.sh 1.0.1
#
# Created by Oscar Esteban
# Source: https://github.com/poldracklab/fmriprep/blob/master/update_changes.sh
#
# Adaptation by Sebastien Tourbier (sebastien.tourbier@alumni.epfl.ch)
#

# Setting      # $ help set
set -u         # Treat unset variables as an error when substituting.
set -x         # Print command traces before executing command.

# Check whether the Upcoming release header is present
head -1 docs/CHANGES.rst | grep -q Upcoming
UPCOMING=$?
if [[ "$UPCOMING" == "0" ]]; then
    head -n3  docs/CHANGES.rst >> docs/newchanges
fi

# Elaborate today's release header
HEADER="$1 ($(date '+%B %d, %Y'))"
echo $HEADER >> docs/newchanges
echo $( printf "%${#HEADER}s" | tr " " "=" ) >> docs/newchanges
echo "" >> docs/newchanges

# Search for PRs since previous release
# git log 3.0.0-beta-20190815..HEAD --pretty='format:  * %b %s'
git log --grep="Merge pull request" `git describe --tags --abbrev=0`..HEAD --pretty='format:  * %b %s' | sed  's/Merge pull request \#\([^\d]*\)\ from\ .*/(\#\1)/' >> docs/newchanges
echo "" >> docs/newchanges
echo "" >> docs/newchanges

# Add back the Upcoming header if it was present
if [[ "$UPCOMING" == "0" ]]; then
    tail -n+4 docs/CHANGES.rst >> docs/newchanges
else
    cat docs/CHANGES.rst >> docs/newchanges
fi

# Replace old CHANGES.rst with new file
mv docs/newchanges docs/CHANGES.rst
