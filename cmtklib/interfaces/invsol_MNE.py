...
class MNEInverseSolutionInputSpec(BaseInterfaceInputSpec):
    ...
    mri_file = traits.File(
		exists=True, desc='Segmented FreeSurfer MR image', mandatory=True)

    # trans_file = traits.File(
    #     exists=True, desc='EEG electrode coordinates coregistered to MRI, -trans.fif', mandatory=True)

    # bem_conductivity = traits.Tuple(desc='Conductivities of inner skull, outer skull, outer skin for head model',(traits.Float,traits.Float,traits.Float)), default_value=(0.3, 0.006, 0.3))

#     snr = traits.Float(
#         desc='Signal to noise ration for inverse solution', mandatory=False, default_value=3.)

# 	lambda2 = traits.Float(
# 		desc='Regularization parameter for inverse solution', mandatory=False, default_value=1./snr**2)
